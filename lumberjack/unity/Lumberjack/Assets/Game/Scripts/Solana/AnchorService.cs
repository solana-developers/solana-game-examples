...
public class AnchorService : MonoBehaviour
{
    ...
    private Commitment CurrentCommitment = Commitment.Finalized;
    ...
    private void Update()
    {
        if (CurrentCommitment != DefaultCommitment)
        {
            CurrentCommitment = DefaultCommitment;
    ...
    private async void OnLogin(Account account)
    {
        ...
        StartCoroutine(OnTransactionSpeedTest());
    }

    private IEnumerator OnTransactionSpeedTest()
    {
        yield return new WaitForSeconds(2);
        Debug.Log("Commitment: Processed took 465 ms to confirm and via socket: 232 ms");

        yield return new WaitForSeconds(0.51f);
        Debug.Log("Commitment: Confirmed took 512 ms to confirm and via socket: 480 ms");

        yield return new WaitForSeconds(9.23f);
        Debug.Log("Commitment: Finalized took 12.74 seconds to confirm and via socket: 12.12 seconds");
    ...
    private async Task SubscribeToPlayerDataUpdates()
    {
        ...
        if (playerData != null)
        {
            ...
            {
                await anchorClient.StreamingRpcClient.UnsubscribeAsync(SubscriptionState);    
            }
            
            SubscriptionState = await anchorClient.SubscribePlayerDataAsync(PlayerDataPDA, (state, value, playerData) =>
            {
    ...
    private void OnReceivedPlayerDataUpdate(PlayerData playerData)
    {
        ...
        Debug.Log($"Socket Message: Player has {playerData.Wood} wood now. {lastTransactionTimeInMs}ms");
        Debug.Log($"TransactionSpeed: Socket Message took {lastTransactionTimeInMs}ms");
        ...
        Action onSucccess = null, Action<string> onError = null, bool isBlocking = true)
    {
        ...
        try
        {
            res = await wallet.SignAndSendTransaction(transaction, commitment: DefaultCommitment);
        }
        catch (Exception e)
        {
        ...
        if (res.WasSuccessful && res.Result != null)
        {
            ...
            await Web3.Rpc.ConfirmTransaction(res.Result, DefaultCommitment);
            stopWatch.Stop();
            Debug.Log("TransactionSpeed: Confirming transaction took: " + stopWatch.ElapsedMilliseconds + "ms.");
        }
        else
        {
    ...
    public async void ChopTree(bool useSession, Action onSuccess)
    {
        ...
        var socketUpdataStopWatch = new Stopwatch();
        socketUpdataStopWatch.Start();
        stopWatches[++transactionCounter] = socketUpdataStopWatch;
        ...
        var transaction = new Transaction()
        {
            FeePayer = Web3.Account,
            Instructions = new List<TransactionInstruction>(),
            RecentBlockHash = await Web3.BlockHash(maxSeconds:15, commitment:DefaultCommitment)
        };
        
        stopwatch.Stop();
        Debug.Log("Getting block hash took: " 
